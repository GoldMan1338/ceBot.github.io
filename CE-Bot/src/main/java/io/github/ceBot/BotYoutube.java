...
class BotYoutube {
    private static final String YOUTUBE_CHANNEL_ID = "UClrHWl3JBBL7oXoaziuj-pQ";
    private static final long NOTIFICATION_CHANNEL = 550777419930402837L;
        ...
        Flux.interval(Duration.ofMinutes(5))
                .flatMap(l -> checkStreaming(client)
                        ...
                        .onErrorResume(throwable -> {
                            ...
                            LoggerFactory.getLogger(Main.class).error("YouTube listener had an uncaught exception!", throwable);
                            return Mono.empty();
                        }))
                .subscribe();
    }
    ...
    private static Mono<Void> updateStatistics(DiscordClient client) {
        return getYoutubeJson(STATISTICS_URL)
                .zipWith(client.getChannelById(Snowflake.of(NOTIFICATION_CHANNEL))
                        ...
                        .flatMap(BotYoutube::getStatisticsCategory))
                .flatMap(tuple -> {
                    JsonNode json = tuple.getT1();
                    Statistics stats;
                    try {
                        stats = Main.mapper.treeToValue(json.path("statistics"), Statistics.class);
    ...
    private static Mono<JsonNode> getYoutubeJson(String url) {
        return Mono.fromCallable(() -> {
            JsonNode json = Main.mapper.readTree(new URL(url));
    ...
    private static Mono<Message> sendNotification(DiscordClient client, JsonNode json, String message) {
        return client.getChannelById(Snowflake.of(NOTIFICATION_CHANNEL))
                ...
                .flatMap(c -> BotUtils.sendMessage(message, c));
    }
