...
public class Tracks extends AudioEventAdapter {
    ...
    Tracks(AudioPlayer player, Snowflake guildId, DiscordClient client) {
        this.player = player;
    ...
    public void onTrackStart(AudioPlayer player, AudioTrack track) {
        Music.getBotConnectedVoiceChannel(client, guildId).hasElement()
                .doOnNext(b -> {
                    if (b) {
                        ServerMusic manager = Music.getGuildManager(client, guildId);
