...
public class Music {
    ...
    private static final Map<Snowflake, ServerMusic> musicManagers;
    ...
    public static synchronized ServerMusic getGuildManager(DiscordClient client, Snowflake guildId) {
        return musicManagers.computeIfAbsent(guildId, k -> new ServerMusic(playerManager, guildId, client));
    }

    public static void skipTrack(DiscordClient client, Snowflake guildId) {
        ServerMusic musicManager = getGuildManager(client, guildId);
    ...
    public static Mono<Boolean> locked(Member member, TextChannel channel) {
        return channel.getGuild().map(Guild::getId)
                ...
                .filterWhen(snowflake -> MusicPerms.getPermission(member).map(perm -> perm == MusicPerms.NONE))
                .flatMap(snowflake -> BotUtils.sendMessage(":lock: Music is currently locked for normal members. Please try again later.", channel))
                .hasElement();
    }
